...
<div style="background: #1a1a2e; border: 1px solid #333; border-radius: 16px; padding: 32px;">
    ...
    <div style="display: flex; gap: 8px; margin-bottom: 24px;">
        ...
        <a href="/register/mechanic" style="flex: 1; padding: 14px; background: #F47920; color: white; text-align: center; border-radius: 8px; font-size: 15px; font-weight: 600; text-decoration: none;">🔧 Mechanic</a>
    </div>
    ...
    <form action="/register" method="POST" style="display: flex; flex-direction: column; gap: 14px;">
        ...
        <div>
            ...
            <input type="text" name="phone" value="{{ old('phone') }}" required placeholder="+964 770 123 4567"
                style="width: 100%; padding: 12px; background: #1f2937; border: 1px solid #4b5563; border-radius: 8px; color: white; font-size: 14px; box-sizing: border-box;">
        </div>

        <div>
            <label style="display: block; font-size: 13px; color: #999; margin-bottom: 4px;">City *</label>
            <input type="text" name="city" value="{{ old('city') }}" required placeholder="Baghdad, Erbil..."
                style="width: 100%; padding: 12px; background: #1f2937; border: 1px solid #4b5563; border-radius: 8px; color: white; font-size: 14px; box-sizing: border-box;">
        </div>

        <div>
            <label style="display: block; font-size: 13px; color: #999; margin-bottom: 6px;">Specialization * (select all that apply)</label>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 6px;">
                @php $specialties = ['engine-diagnostics' => '🔧 Engine', 'electrical' => '⚡ Electrical', 'diagnostics' => '📱 Diagnostics', 'brakes' => '🛞 Brakes', 'suspension' => '🔩 Suspension', 'transmission' => '🔄 Transmission', 'ac' => '❄️ AC & Heating', 'oil' => '🛢️ Oil & Fluids', 'body' => '🎨 Body & Paint', 'tuning' => '💻 ECU Tuning']; @endphp
                @foreach($specialties as $key => $label)
                <label style="display: flex; align-items: center; gap: 6px; padding: 8px 10px; background: #1f2937; border: 1px solid #4b5563; border-radius: 6px; cursor: pointer; font-size: 12px; color: #ccc;">
        ...
        @if($errors->any())
        <div style="background: rgba(239,68,68,0.2); border: 1px solid rgba(239,68,68,0.3); border-radius: 8px; padding: 12px;">
        ...
        <button type="submit" style="width: 100%; padding: 14px; background: #F47920; color: white; border: none; border-radius: 8px; font-weight: bold; font-size: 15px; cursor: pointer;">
